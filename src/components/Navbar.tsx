...
import { Activity, User, LogOut, Users, Calendar, Utensils, ShieldCheck, Crown, Sparkles, Flame, HeartPulse } from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({ user, activeTab, setActiveTab, onLogout }) => {
  ...
  return (
    <header className="glass-panel" style={{ borderRadius: 0, borderTop: 0, borderLeft: 0, borderRight: 0, padding: '14px 28px', borderBottom: '1px solid rgba(59, 130, 246, 0.2)' }}>
        ...
        {user && (
          <nav style={{ display: 'flex', gap: '6px', background: 'rgba(15, 23, 42, 0.7)', padding: '4px', borderRadius: '12px', border: '1px solid rgba(255, 255, 255, 0.08)' }}>
            ...
            <button
              ...
              style={{ padding: '8px 16px', fontSize: '0.875rem' }}
            >
              <Utensils size={16} /> Planos & Dietas
            </button>
          </nav>
        )}
        ...
        {user ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            <div style={{ 
              ...
              alignItems: 'center', 
              gap: '10px', 
              background: isMaster ? 'rgba(245, 158, 11, 0.08)' : 'rgba(37, 99, 235, 0.08)', 
              padding: '6px 14px', 
              borderRadius: '9999px', 
              border: isMaster ? '1px solid rgba(245, 158, 11, 0.3)' : '1px solid rgba(37, 99, 235, 0.3)' 
            }}>
              <div style={{ 
                background: isMaster ? 'rgba(245, 158, 11, 0.2)' : 'rgba(37, 99, 235, 0.2)', 
                borderRadius: '50%', 
                padding: '6px', 
